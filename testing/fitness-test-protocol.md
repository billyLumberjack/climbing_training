...
> **v2.2 change vs v2.1:** the finger deadlift is now a _formal_ test — "Massimale stacco da
...
### 2. 3RM weighted pull-up _(kept as-is)_
...
> _Kept deliberately: 3RM is safer and more repeatable for self-testing than the IRCRA 1RM.
> Note pull-up strength is only a moderate predictor of climbing — track it, don't over-weight it._

### 3. AMRAP pull-ups _(kept as-is)_

Bodyweight, max clean reps. Done straight after the 3RM while warm.

### 4. MVC7 — max finger strength _(refined: grip now fixed)_
...
### 5. RFD hang — rate of force development _(NEW — requires a load cell / Tindeq)_

On the 20 mm half-crimp, one hand, pull to max as **fast** as possible from rest; record peak
RFD (or force at 200 ms) from the device. _Skip if you have no load cell._

> _Added because RFD is one of the clearest separators of elite vs sub-elite climbers and is
> exactly what a static max hang misses. Nearly free once a load cell is on hand for the CF test._

### 6. Unilateral MVC7 — asymmetry check _(NEW — do at least occasionally)_

Same as MVC7 but **one hand at a time** (pulley-assisted as needed), left and right. Flag any
gap > ~10% between sides.

> _Added because bilateral hangs hide left/right imbalances that precede finger injuries._

### 7. Finger-flexor Critical Force (CF) — fatigue resistance _(REPLACED — this is the big fix)_
...
2. **4 minutes** of **7:3** repeaters (7″ max half-crimp pull : 3″ rest), one hand, max force
   _every_ rep, with live force feedback on screen. No forearm shaking during rest.
3. From the trace:
...
### 8. Finger deadlift 1RM — max finger strength, pulling vector _(v2.2: now a formal test)_
...
- Record the **max load (kg)** and its **relative value (%BW)**.
  > _Reclassified from v2.1: I had wrongly logged this as a general barbell deadlift. v2.2 makes
...
| Rested? (Y/N)                                                 | —                     | —                     | —               | yes                               |
| **Bodyweight** (kg)                                           | 65                    | 64                    | 63              | 65 vestito con scarpe             |
| **3RM pull-up** — overload (kg)                               | 29                    | 22.5                  | 24.5            | 20kg                              |
| **AMRAP pull-ups** (reps)                                     | 15                    | 12                    | 15              | 13                                |
| **MVC7** — overload ± (kg)                                    | +28                   | +30.3                 | +31.5           | +30.5                             |
| MVC7 — Total Load CC (kg)                                     | 93                    | 94.3                  | 94.5            | 95.5                              |
| MVC7 — relative (%BW)                                         | 143%                  | 147%                  | 150%            | 147%                              |
| MVC7 confirmed? (Y/N)                                         | —                     | —                     | —               | yes                               |
| **Finger deadlift 1RM** (stacco dita, 20 mm, semi-crimp) (kg) | —                     | —                     | 46.3            | 48kg                              |
| Finger deadlift — relative (%BW)                              | —                     | —                     | 73%             | 73%                               |
| **RFD** peak (N/s or device unit)                             | —                     | —                     | —               | 2506 N/s sx ; 3284 N/s dx         |
| **Unilateral MVC7 L** (kg CC)                                 | —                     | —                     | —               | -                                 |
| **Unilateral MVC7 R** (kg CC)                                 | —                     | —                     | —               | -                                 |
| L/R asymmetry (%)                                             | —                     | —                     | —               | -                                 |
| **CF** (kg) _[all-out]_                                       | —                     | —                     | —               | 18.67kg sx ; 15.85 dx             |
| **W′** (kg·s, /kg BW) _[all-out]_                             | —                     | —                     | —               | 762.6 kg\*s sx ; 1256.5kg dx      |
| CC80% load / time-to-exhaustion _[fallback]_                  | 74.4 / 2′ (12 rep)    | 75.4 / 1′40″ (10 rep) | 75.6 / (10 rep) | -                                 |
| CC60% load / time-to-exhaustion _[fallback]_                  | 55.8 / 5′30″ (33 rep) | 56.6 / 3′40″ (22 rep) | 56.7 / (23 rep) | -                                 |
| CC45% load / time (cap 10′) _[fallback]_                      | 41.85 / not done      | not done              | not done        | -                                 |
| Notes / video links                                           |                       |                       |                 |                                   |
...
**Test 4 read (2026-09-11)** — raw CF traces analyzed in full; device CF/W′ verified against
the trace (CF = mean force of last 6 contractions; W′ = impulse above CF, both reproduced
exactly), and the 33–40% force decline with a plateaued last 6 reps confirms W′ was depleted →
**valid test**. Headlines: (1) **first valid CF baseline** — L 18.67 kg / 28.7% BW, R 15.85 kg /
24.4% BW; CF%BW is the single best predictor of sport-climbing grade (Giles 2021, ~61% of
variance), so it's the #1 training lever for the autumn on-sight 7a+ goal, with clear headroom.
(2) **Right-hand CF deficit ~15% below left** (above the protocol's 10% flag) — right hand is
power-biased (RFD 3284 vs 2506 N/s, W′ 1256 vs 762 kg·s) but endurance-limited; it's the
fatigue bottleneck on sustained routes → right-hand bias in the CF repeaters next block.
(3) **MVC7 maintained** (~147–150% BW) and **finger deadlift up** (46.3→48 kg) through three
non-strength mesos — fingers resilient. (4) **3RM pull-up eroded to 20 kg** (lowest ever;
29→22.5→24.5→20) — not the on-sight limiter, but arrest it with a modest pull-rebuild thread.
%BW figures use the 65 kg clothed+shoes denominator (true BW ~63 kg), so they're marginally
conservative. Drives Meso 5 → `orchestrator/plans/2026_autunno_AVista.md`.
...
- **W′** — work capacity available _above_ CF (the "battery" for hard, above-threshold efforts).
